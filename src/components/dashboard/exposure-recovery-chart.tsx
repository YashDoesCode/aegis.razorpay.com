...
export function ExposureRecoveryChart({
  ...
  const { exposurePath, recoveryPath, exposureArea, recoveryArea, maxVal, coordinates } = useMemo(() => {
    ...
    const coords = points.map((p, i) => {
      ...
      const yExp = height - (p.exposurePaise / max) * (height - 40);
      const yRec = height - (p.recoveredPaise / max) * (height - 40);
  ...
  const updateNearestPoint = useCallback((clientX: number) => {
    ...
    const targetSvgX = 24 + relativeX * (1000 - 48);
  ...
  return (
    <div
      ...
      aria-label="Interactive Dispute Exposure and Recovery Chart"
    >
      <div className="flex flex-wrap items-start justify-between gap-3 pb-3 border-b border-border/60">
        <div>
          <div className="flex items-center gap-2">
            <span className="text-xs font-medium text-foreground tracking-tight">
              Dispute Exposure vs Recovery Trend
            </span>
          ...
          <div className="flex flex-wrap items-baseline gap-4 mt-1.5">
            <div>
              ...
              <span className="text-lg sm:text-xl font-semibold font-mono text-foreground tabular-nums ml-2">
                {totalExposure}
              </span>
            </div>
            ...
            <div>
              ...
              <span className="text-xs font-mono text-emerald-600 dark:text-emerald-400 ml-1.5 font-medium">
                (+18.2%)
              </span>
            </div>
          </div>
        ...
        <div
          className="flex items-center gap-0.5 bg-muted/50 p-0.5 rounded-lg border border-border text-xs font-medium text-muted-foreground shadow-xs shrink-0"
          role="tablist"
      ...
      <div
        ...
        className="relative w-full h-52 sm:h-64 mt-2 select-none cursor-crosshair touch-none"
      >
        <div className="absolute inset-x-0 top-3 border-b border-dashed border-border/70 flex items-center justify-between text-[10px] text-muted-foreground font-mono pointer-events-none">
          <span>₹{(maxVal / 10000000).toFixed(1)}L</span>
          <span className="text-[9px] uppercase tracking-wide text-muted-foreground font-sans">
            Max Exposure
          </span>
        </div>

        <div className="absolute inset-x-0 top-24 border-b border-dashed border-border/70 flex items-center justify-between text-[10px] text-muted-foreground font-mono pointer-events-none">
        ...
        >
          <defs>
            <linearGradient id="expGrad" x1="0" x2="0" y1="0" y2="1">
              <stop offset="0%" stopColor="currentColor" className="text-foreground" stopOpacity="0.06" />
        ...
        {activePoint && (
          <div className="absolute right-3 sm:right-6 top-2 bg-card text-foreground text-xs p-3 rounded-lg shadow-md min-w-[210px] border border-border z-10 pointer-events-none animate-in fade-in duration-100">
            ...
            <div className="flex items-center justify-between pt-1.5 text-xs font-mono">
              <span className="text-muted-foreground flex items-center gap-1.5 font-sans text-[11px]">
                <span className="w-1.5 h-1.5 rounded-full bg-muted-foreground" /> Exposure
              </span>
      ...
      <div className="flex items-center justify-between text-xs text-muted-foreground pt-2.5 border-t border-border font-mono overflow-x-auto">
        {points.map((p, idx) => (
          <span
            key={idx}
